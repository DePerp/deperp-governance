...
describe("Governor Bravo", function () {
  async function deployFixtures() {
    ...
    const { governorAlpha, timelock, deperp } = await setupGovernorAlpha();
    ...
    return { owner, otherAccount, governorBravo, deperp };
  }
  ...
  describe("Initialize", function () {
    ...
    it("Error: invalid deperp", async function () {
      const GovernorBravoDelegator = await ethers.getContractFactory(
        "GovernorBravoDelegator"
      );
      const GovernorBravoDelegate = await ethers.getContractFactory(
      ...
      ).to.be.revertedWith("GovernorBravo::initialize: invalid deperp address");
    });
  ...
  describe("Propose", function () {
    ...
    it("Error: bravo not active", async function () {
      const { timelock, deperp } = await setupGovernorAlpha();
  ...
  describe("Cancel", function () {
    ...
    it("Happy Path: below threshold", async function () {
      const { governorBravo, deperp, otherAccount } = await loadFixture(
        deployFixtures
      );
      const proposalId = await proposeAndPass(governorBravo);

      await deperp.delegate(otherAccount);
    ...
    it("Error: cancel executed proposal", async function () {
      const { governorBravo, owner } = await loadFixture(deployFixtures);
      const tx = { to: await governorBravo.timelock(), value: 1000000 };
    ...
    describe("Whitelisted", function () {
      ...
      it("Error: whitelisted proposer above threshold", async function () {
        const { governorBravo, owner, otherAccount, deperp } = await loadFixture(
          deployFixtures
        );
  ...
  describe("State", async function () {
    ...
    it("Defeated: quorum", async function () {
      ...
      await deperp.transfer(otherAccount, BigInt("100000"));
      await deperp.connect(otherAccount).delegate(otherAccount);
    ...
    it("Defeated: against", async function () {
      ...
      await deperp.transfer(
        otherAccount,
        BigInt("10000000") * BigInt("10") ** BigInt("18") // quorum
      );
      await deperp.connect(otherAccount).delegate(otherAccount);
  ...
  describe("Admin Functions", function () {
    ...
    describe("Set Proposal Threshold", function () {
      ...
      it("Invalid proposal threshold", async function () {
        const { governorBravo } = await loadFixture(deployFixtures);
        await expect(
          governorBravo._setProposalThreshold(1000000)
        ).to.be.revertedWith(
          "GovernorBravo::_setProposalThreshold: invalid proposal threshold"
        );
        await expect(
          governorBravo._setProposalThreshold(10000001n * 10n ** 18n)
        ).to.be.revertedWith(
          "GovernorBravo::_setProposalThreshold: invalid proposal threshold"
        );
      });
      ...
      it("Happy Path", async function () {
        const { governorBravo } = await loadFixture(deployFixtures);
        await expect(governorBravo._setProposalThreshold(1000001n * 10n ** 18n))
          .to.emit(governorBravo, "ProposalThresholdSet")
          .withArgs(1000000n * 10n ** 18n, 1000001n * 10n ** 18n);
      });
    });
